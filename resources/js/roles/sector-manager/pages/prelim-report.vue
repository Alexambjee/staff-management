<template>
  <div >
  <!-- navbar -->
    <Navbar  :messagePath="messagePath"/>
       <div id="layoutSidenav">
        <Sidebar  :messagePath="messagePath" :username="username" :manualPath="manualPath" :channel="channel"/>
          <div id="layoutSidenav_content">
              <main>
                  <div class="container-fluid px-4">
                      <h1 class="mt-4">PRELIMINARY REPORTS</h1>
                      <div class="breadcrumb mb-4 mt-3">
                           <Breadcrumb>
                            <BreadcrumbItem to="/Sector-manager" class="breadcrumb-item">Dashboard</BreadcrumbItem>
                            <BreadcrumbItem to="/Sector-manager/Preliminary-reports" 
                 class="breadcrumb-item active">Preliminary Reports  </BreadcrumbItem>
                        </Breadcrumb>
                      </div>


                        <div class="card shadow mb-4">
                            <div class="card-header bg-dark text-muted">
                                <i class="fas fa-table me-1"></i>
                                Preliminary Reports 
                            </div>
                              <!-- include case table -->
                              <caseTable  :path="path" :action="action" :caseStatus="caseStatus" 
                              :username="username" :column="column" />
                            <!-- include case table ends-->

                        </div>

                  </div>
              </main>
            <!-- footer -->
            <Footer></Footer>
          </div>
        </div>
  </div>
      <!-- if not authorized -->
   
  <!-- if not authorized  ends-->

</template>
<script>
import Footer from '../../../components/footer.vue'
import Sidebar from '../../../components/sidebar.vue'
import Navbar from '../../../components/navbar.vue'
import caseTable from '../../../components/pages/caseTable.vue'

export default {
  props: ['user'],
  data(){
    return{
      caseStatus:[2],
      path:'/Sector-manager/Preliminary-reports/',
      column:'SM_PNO',
      username:this.$store.state.user.USER_NAME,
      role: this.$store.state.userRole,
      action:'Details',
      channel: '/Sector-manager/progress-reports',
       messagePath:'/Sector-manager/messages',
       manualPath:'/Sector-manager/Manual-cases',
    }

  },
  components : {
    Footer,
    Sidebar,
    Navbar,
    caseTable
  },
  created() {
    //  if (!this.$store.state.user) {
    //   this.warning("Access Denied. You Don't Have Enough Permissions to Access This Route!")
    //   window.location = '/'
    // }
    // else {
    // }
  }



}
</script>
